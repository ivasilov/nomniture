...
p.sendRequest = function(method, parameters, callback){
	...
	request.post({url:"https://"+ this.environment+this.path+"?method="+method, form: parameters}, function (error, response, body) {
		...
		// Add a special case when the body contains an error info (for example, calling Status.Get with a report id for
		// an incomplete report). In this case, the server returns 400 status.
		if (body && response.statusCode === 400) {
			return callback(null,body);
		}

		if (response.statusCode != 200) {
			return callback(new Error("Server returned " + response.statusCode + " code."));
		}
